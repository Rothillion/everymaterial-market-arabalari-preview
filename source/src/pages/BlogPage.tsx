...
import { blogContent, blogArticleContent, type BlogPost } from "../content/blog";
...
function FeaturedCard({ post, lang, readMoreLabel, excerpt }: { post: BlogPost; lang: Lang; readMoreLabel: string; excerpt: string | null }) {
  ...
  return (
    ...
    >
      ...
      <div className="w-[92%] md:w-1/2 self-center bg-white border border-navy/10 rounded-3xl p-7 md:p-9 flex flex-col justify-center relative z-10 -mt-14 md:mt-0 md:-ms-20 shadow-xl group-hover:border-accent/40 transition-colors">
...
function PostCard({ post, lang, readMoreLabel }: { post: BlogPost; lang: Lang; readMoreLabel: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="w-[90%] self-center bg-white border border-navy/10 rounded-2xl p-5 flex flex-col flex-grow relative z-10 -mt-10 shadow-lg group-hover:border-accent/40 transition-colors">
...
export function BlogPage({ lang }: { lang: Lang }) {
  ...
  const featured = c.posts.find((p) => p.hasDetail) ?? c.posts[0];
  const rest = c.posts.filter((p) => p.slug !== featured.slug);
          ...
          {c.filters.map((f, i) => (
            <span
              ...
              className={`rounded-full px-5 py-2.5 text-[13px] font-bold ${
                i === 0 ? "bg-accent-deep text-white shadow-md" : "bg-white border border-navy/15 text-[#5b6b7d]"
              }`}
            >
              {f}
            </span>
